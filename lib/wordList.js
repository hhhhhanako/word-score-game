...
var Word_List = (function() {
    ...
    function regTest(reg,character){
        if(character!=='_'){
            
            for(var i=0;i<wordBank[character].length;i++){
                if(reg.test(wordBank[character][i])){
                    return true;
                }
            }
        }else{
            ...
            for(c in wordBank){
                for(var i=0;i<wordBank[c].length;i++){
                    if(reg.test(wordBank[c][i])){
                        return true;
                    }
                }
            }
    ...
    publicInt.test=regTest;
    publicInt.getwordBank=getwordBank;
